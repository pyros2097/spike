...
func Fade() Interpolation {
	return func(a float32) float32 {
		utils.ClampFloat32(a*a*a*(a*(a*6-15)+10), 0, 1)
		return a
	}
}
...
func Sine() Interpolation {
	return func(a float32) float32 {
		// (1 - MathUtils.cos(a*MathUtils.utils.PI)) / 2
		return a
	}
}
...
func SineOut() Interpolation {
	return func(a float32) float32 {
		// MathUtils.sin(a * MathUtils.utils.PI / 2)
		return a
	}
}
...
func Elastic(value, power float32, bounces int, scale float32) Interpolation {
	if bounces == 0 {
		bounces = int(utils.PI * float32(bounces*1))
	} else {
		bounces = int(utils.PI * float32(bounces*-1))
	}
	return func(a float32) float32 {
